<template>
    <li class="my-2">
        <form class="flex items-center" @submit.prevent="updateCategory(category)">
            <div @click="toggle" :class="{'cursor-pointer': isFolder && category.disabled}" class="flex items-center justify-between bg-gray-100 w-80">
                <input :class="{'!border-teal-600 shadow-inner':category.disabled == false , 'cursor-pointer': isFolder && category.disabled }" type="text" v-model="category.name" class=" border-b border-transparent outline-none py-0.5 px-2" :disabled="category.disabled">
